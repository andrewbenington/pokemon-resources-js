...
export const getLocationString = (
    ...
    const locationBlock =
        locations[Math.floor(index / multiplier) * multiplier];
    if (locationBlock) {
...
export function getMetLocation(
    ...
    if (
        origin === GameOfOrigin.ColosseumXD ||
        fileFormat === 'COLOPKM' ||
        fileFormat === 'XDPKM'
    ) {
    ...
    if ((origin && isAlola(origin)) || fileFormat === 'PK7') {
        return Gen7AlolaLocations[Math.floor(index / 10000) * 10000][
            index % 10000
        ];
    }

    if ((origin && isLetsGo(origin)) || fileFormat === 'PB7') {
        return Gen7KantoLocations[Math.floor(index / 10000) * 10000][
            index % 10000
        ];
    }

    if ((origin && isGalar(origin)) || fileFormat === 'PK8') {
        return Gen8GalarLocations[Math.floor(index / 10000) * 10000][
            index % 10000
        ];
    }

    if ((origin && isBDSP(origin)) || fileFormat === 'PB8') {
        return Gen8SinnohLocations[Math.floor(index / 10000) * 10000][
            index % 10000
        ];
    }

    if (origin === GameOfOrigin.LegendsArceus || fileFormat === 'PA8') {
        return Gen8HisuiLocations[Math.floor(index / 10000) * 10000][
            index % 10000
        ];
    }
